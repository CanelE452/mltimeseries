...
PLAN = ROOT / "_docs/notes/tsfm_topics/20a_peft_fullft_memory_recovery_20260909.md"
...
def parent_hashes():
    ...
    predecessor = read_json(ROOT/"runs/peft_fullft_reference_v1/study_contract.json")
    for name in ("core_hashes", "holdout_hashes", "raw_hashes"):
        for key, value in predecessor[name].items():
            ...
            output[key] = value
    for path in (ROOT/"runs/peft_fullft_reference_v1").rglob("*"):
        if path.is_file():
            output[relative(path)] = digest(path)
