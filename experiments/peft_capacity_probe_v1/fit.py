...
from experiments.peft_mechanism_diagnostics_v1.controlled_fit import (
    check, predict, sha, save, shared, native,
    parameter_digest, snapshot_trainable, restore_trainable,
)

from datetime import datetime, timezone
from experiments.peft_capacity_probe_v1.model import Controlled
...
def main():
    ...
    initial_state = snapshot_trainable(model)
    ...
    history = [{'step': 0, 'score': initial_score, 'utc': datetime.now(timezone.utc).isoformat()}]
    ...
    for step in range(1, steps + 1):
        ...
        if step in checkpoints:
            ...
            np.savez_compressed(out / f'point_{step}.npz', prediction=prediction, target=panel.targets('val'), scale=panel.fit_std[panel.target_indices], quantiles=panel.quantiles)
            history.append({'step': step, 'score': score, 'utc': datetime.now(timezone.utc).isoformat()})
    ...
    assert parameter_digest(model, trainable=False) == frozen
    deltas = {n: float((p.detach().cpu()-initial_state[n]).abs().max()) for n,p in model.named_parameters() if p.requires_grad}
    if job['arm'] == 'WIDE':
    ...
    save(out / 'result.json', {'completed': True, 'job': job, 'regimes': records, 'history': history, 'trainable': count, 'trainable_tensor_max_changes': deltas, 'initial_head_hash': model.initial_head_hash, 'identity_error': identity, 'frozen_hash': frozen, 'frozen_verified': True, 'steps': steps, 'minimum_commit_gib': minimum, 'max_cuda_allocated_gib': torch.cuda.max_memory_allocated()/2**30, 'seconds': time.perf_counter()-started, 'contract_sha256': sha(args.contract), 'holdout_opened': False, 'sample_index_sha256': __import__('hashlib').sha256(samples.tobytes()).hexdigest()})
