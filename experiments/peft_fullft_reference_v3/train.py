...
def memory_phase(output, phase):
    from experiments.peft_adaptation_scope_v1.guard import _sample_resources
    system = _sample_resources(None, None)
    process = psutil.Process().memory_info()
    shared.log(output,"memory_phase",phase=phase,available_commit_gib=system["available_commit_gib"],
               available_ram_gib=system["available_ram_gib"],rss_bytes=process.rss,
               private_commit_bytes=getattr(process,"private",None),
               cuda_allocated_bytes=torch.cuda.memory_allocated(),cuda_reserved_bytes=torch.cuda.memory_reserved())


def check_commit_headroom(output, phase):
    available = _available_commit() / 1024**3
    if available < 6.:
        memory_phase(output, phase)
        raise RuntimeError(f"available_commit_below_limit at {phase}: {available:.6f} GiB < 6 GiB")
    return available
...
def execute(args):
    ...
    tick = time.perf_counter()
    best_store = modeling.MappedBestState(model,output/"best_trainable_working.float32.bin")
    best_state = best_store.state
    timings["mapped_best_capture"] = time.perf_counter()-tick
    memory_phase(output,"initial_snapshot")
    history = [{"step":0,"val_score":best}]
    shared.log(output,"validation",**history[-1])
    durations, gradients, losses = [], [], []
    minimum_active_commit = float("inf")
    for step in range(1,steps+1):
        ...
        for offset in (0,4):
            ...
            phase = f"post_backward_step_{step}_micro_{offset//4+1}"
            minimum_active_commit = min(minimum_active_commit,check_commit_headroom(output,phase))
        ...
        phase = f"post_optimizer_before_cleanup_step_{step}"
        minimum_active_commit = min(minimum_active_commit,check_commit_headroom(output,phase))
        if step == 1 or step == steps:
            memory_phase(output,phase)
        optimizer.zero_grad(set_to_none=True)
        ...
        minimum_active_commit = min(minimum_active_commit,check_commit_headroom(output,f"post_optimizer_step_{step}"))
        durations.append(time.perf_counter()-tick)
        losses.append(total_loss)
        if step == 1 or step == steps:
            memory_phase(output,f"optimizer_step_{step}")
        if step == 1:
        ...
        if step % interval == 0 or step == steps:
            ...
            if metrics["score"] < best:
                ...
                best_store.capture(model)
                timings["mapped_best_capture"] += time.perf_counter()-capture_started
            timings["validation"] += time.perf_counter()-tick
    ...
    memory_phase(output,"before_optimizer_cleanup")
    optimizer.zero_grad(set_to_none=True)
    del optimizer, parameters, context, batch_target, groups, norm, loss, loc, scale, gradient
    gc.collect()
    torch.cuda.synchronize()
    torch.cuda.empty_cache()
    memory_phase(output,"after_optimizer_cleanup")
    tick = time.perf_counter()
    ...
    memory_phase(output,"after_checkpoint_restore")
    ...
    audits.update(zero_update_identity=True,zero_update_max_abs=identity_error,trainable_map_verified=True,
                  ...
                  active_commit_checks_verified=True,active_commit_checks=steps*4,
                  minimum_sampled_active_commit_gib=minimum_active_commit)
    ...
    result = {"completed":True,"study":STUDY,"stage":"fit","dataset":args.dataset,"arm":args.arm,
              ...
              "mapped_working_file":str(best_store.path),"mapped_working_bytes":best_store.nbytes,
              "mapped_working_sha256":digest(best_store.path),
              "rss_sample_max_bytes":rss_max,"rss_sampling":"child process RSS sampled at step boundaries and final; not continuous peak",
              "audits":audits,"checkpoint_policy":"in-place FP32 mapped best state; single final checkpoint write; mmap CPU reload; working file retained; no optimizer/RNG resume state"}
