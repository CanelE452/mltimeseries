...
def verify_fit(path, guard, contract, dataset, arm, lr, seed, smoke):
    ...
    for flag in ("zero_update_identity", "trainable_map_verified", "optimizer_exact_parameter_set",
                 "finite_nonzero_gradient_verified", "checkpoint_reload_verified", "active_commit_checks_verified"):
        if result["audits"].get(flag) is not True:
            raise AssertionError(f"Missing required fit audit: {flag}")
    if (result["audits"]["active_commit_checks"] != result["steps_completed"]*4
            or result["audits"]["minimum_sampled_active_commit_gib"] < 6.
            or result["mapped_working_sha256"] != digest(result["mapped_working_file"])
            or result["mapped_working_bytes"] != result["trainable"]*4):
        raise AssertionError("Active memory or mapped best-state audit differs")
    if arm == "FULL_FT":
