...
PLAN = ROOT / "_docs/notes/tsfm_topics/20b_peft_fullft_active_memory_recovery_20260909.md"
...
def parent_hashes():
    ...
    for version in ("peft_fullft_reference_v1", "peft_fullft_reference_v2"):
        predecessor = read_json(ROOT/"runs"/version/"study_contract.json")
        for name in ("core_hashes", "holdout_hashes", "raw_hashes"):
            for key, value in predecessor[name].items():
                if key in output and output[key] != value:
                    raise AssertionError("Predecessor provenance conflicts")
                output[key] = value
        for path in (ROOT/"runs"/version).rglob("*"):
            if path.is_file():
                output[relative(path)] = digest(path)
...
def freeze():
    ...
    original_plan = ROOT/"_docs/notes/tsfm_topics/20_peft_fullft_reference_plan_20260909.md"
    core[relative(original_plan)] = digest(original_plan)
    sources = []
    ...
    verify_hashes(raw)
    fit_audit = ROOT/"runs/peft_fullft_reference_v1/prepared/independent_fit_audit.json"
    if read_json(fit_audit)["passed"] is not True:
        raise AssertionError("Independent fit chronology/statistics audit must pass")
    saved = {"completed": True, "study": STUDY, "created_at_utc": datetime.now(timezone.utc).isoformat(),
             ...
             "independent_fit_audit_path": relative(fit_audit), "independent_fit_audit_sha256": digest(fit_audit),
