...
PRIOR_INDEPENDENT_FIT_AUDIT = "runs/peft_fullft_reference_v1/prepared/independent_fit_audit.json"


def _root_path(root, value):
    path = Path(value)
    return path if path.is_absolute() else Path(root) / path


def _canonical_boundaries(value):
    return {name: list(pair) for name, pair in value.items()}


def verify_fixed_chronology(contract):
    output = {}
    for dataset, entry in contract["datasets"].items():
        if dataset not in EXPECTED_BOUNDARIES:
            raise AssertionError("Prepared chronology includes an unexpected dataset")
        boundaries = _canonical_boundaries(entry.get("boundaries", {}))
        if boundaries != EXPECTED_BOUNDARIES[dataset]:
            raise AssertionError("Prepared chronology differs from the fixed plan20 boundaries")
        if entry.get("origin_counts") != EXPECTED_ORIGIN_COUNTS:
            raise AssertionError("Prepared chronology differs from train90/V30/C20/E80")
        if entry.get("boundary_indices") != EXPECTED_BOUNDARY_INDICES:
            raise AssertionError("Prepared chronology boundary indices differ from the fixed 242-day block")
        output[dataset] = {"boundaries": boundaries, "origin_counts": entry["origin_counts"],
                           "boundary_indices": entry["boundary_indices"]}
    return output


def verify_prior_independent_fit_audit(root, contract, document):
    protected = contract.get("protected_hashes", {})
    path_key = contract.get("independent_fit_audit_path")
    expected_sha = contract.get("independent_fit_audit_sha256")
    if path_key is not None or expected_sha is not None:
        if not path_key or not expected_sha:
            raise AssertionError("Frozen independent fit audit path/hash pair is incomplete")
        protected_sha = protected.get(path_key)
        if protected_sha != expected_sha:
            raise AssertionError("Frozen independent fit audit hash is not pinned in protected_hashes")
    else:
        matches = [key for key in protected if key.replace("\\", "/").endswith(PRIOR_INDEPENDENT_FIT_AUDIT)]
        if len(matches) != 1:
            raise AssertionError("Frozen independent fit audit hash is required")
        path_key = matches[0]
        expected_sha = protected[path_key]
    if not path_key.replace("\\", "/").endswith(PRIOR_INDEPENDENT_FIT_AUDIT):
        raise AssertionError("Frozen independent fit audit hash is required")
    audit = document(_root_path(root, path_key), expected_sha)
    if audit.get("completed") is not True or audit.get("passed") is not True:
        raise AssertionError("Frozen independent fit audit did not pass")
    for dataset, expected in EXPECTED_BOUNDARIES.items():
        recorded = audit.get("datasets", {}).get(dataset, {})
        if recorded and _canonical_boundaries(recorded.get("boundaries", {})) != expected:
            raise AssertionError("Frozen independent fit audit boundaries differ from plan20")
        if recorded and recorded.get("origin_counts") != EXPECTED_ORIGIN_COUNTS:
            raise AssertionError("Frozen independent fit audit origin counts differ from plan20")
    return {"path": path_key, "sha256": expected_sha}
...
def cost_summary(records):
    ...
    for arm in ("HEAD_ONLY", "LORA", "FULL_FT"):
        ...
        output[arm] = {"fit_count": len(group), "selected_fit_count": len(selected),
                       ...
                       "selected_fit_seconds_sum": sum(row["fit_seconds"] for row in selected),
                       "selected_guard_seconds_sum": sum(row["guard_seconds"] for row in selected),
...
def execute(root=ROOT):
    ...
    independent_fit_audit = verify_prior_independent_fit_audit(root, contract, document)
    fixed_chronology = verify_fixed_chronology(contract)
    ...
    selected_by_forecast_key = {(row["dataset"], row["arm"], row["seed"]): row for row in selection["selected"]}
    if len(selected_by_forecast_key) != len(selection["selected"]):
        raise AssertionError("Selected fit identities are not unique by dataset/arm/seed")
    ...
    for dataset, entry in contract["datasets"].items():
        ...
        for letter, source, split, count in (("V", fit, "val", 30), ("C", future, "cal", 20), ("E", future, "eval", 80)):
            ...
            left, right = fixed_chronology[dataset]["boundary_indices"][split]
            if int(origins[0]) != left or int(origins[-1] + 48) != right:
                raise AssertionError("Prepared split origins differ from fixed chronology")
    ...
    for entry in completed["forecasts"]:
        ...
        if arm != "F0":
            selected_fit = selected_by_forecast_key.get((dataset, arm, result["seed"]))
            actual_fit_dir = Path(result.get("fit_dir", "")).resolve()
            expected_fit_dir = (root / selected_fit["fit_dir"]).resolve() if selected_fit else None
            if (selected_fit is None or actual_fit_dir != expected_fit_dir
                    or result.get("fit_result_sha256") != selected_fit["fit_result_sha256"]
                    or result.get("checkpoint_sha256") != selected_fit["checkpoint_sha256"]):
                raise AssertionError("Forecast is not linked to the exact selected fit")
            fit, cost = fits[str(actual_fit_dir)]
            if (not cost["selected"] or fit["checkpoint_sha256"] != result["checkpoint_sha256"]
                    or fit["lr"] != selected_fit["lr"] or fit["trainable"] != EXPECTED_PARAMETERS[arm]
                    or (fit["dataset"], fit["arm"], fit["seed"]) != (dataset, arm, result["seed"])):
                raise AssertionError("Forecast is not linked to the exact selected fit")
    ...
    for dataset in contract["datasets"]:
        ...
        expected_simple = {"dataset": dataset, "completed": True, "stage": "fit",
                           ...
                           "selection_sha256": entry["selection_sha256"], "model_sha256": entry["model_sha256"]}
        if (any(result.get(key) != value for key, value in expected_simple.items())
                or choice.get("completed") is not True or choice.get("dataset") != dataset
                or choice.get("contract_sha256") != contract_hash
                or choice.get("fit_data_sha256") != contract["datasets"][dataset]["fit_data_sha256"]
                or choice["model_sha256"] != entry["model_sha256"] or result["selection_sha256"] != entry["selection_sha256"]
                ...
                or forecast.get("holdout_data_sha256") != contract["datasets"][dataset]["holdout_data_sha256"]
                or forecast.get("uses_calibration_for_fitting") is not False
                or choice["uses_calibration_labels"] is not False or choice["uses_evaluation_labels"] is not False):
            raise AssertionError("Simple reference identity/provenance differs")
        ...
        ridge_candidates = [row for row in choice["candidates"] if row.get("method") == "CONTEXT_RIDGE"]
        if (len(candidate_scores) != result["candidate_count"] or selected_index != choice["selected_index"]
                or saved_v["candidate_predictions"].shape[0] != result["candidate_count"]
                or result["ridge_oof_fits"] != 2 * len(ridge_candidates)
                or result["ridge_final_fits"] != len(ridge_candidates)):
        ...
        record = {"arm": "SIMPLE", "seed": None, "metadata": {"path": str(directory), "trainable": None,
                   ...
                   "optimizer_seconds_per_step": None, "fit_seconds": None,
                   "hpo_fit_seconds": result["wall_seconds"], "selected_fit_seconds": None,
                   "selected_fit_time_unavailable": True,
        ...
        records[dataset].append(record)
        simple_costs[dataset] = {"candidate_count": result["candidate_count"],
                                 "ridge_oof_fits": result["ridge_oof_fits"],
                                 "ridge_final_fits": result["ridge_final_fits"],
                                 "hpo_fit_seconds": result["wall_seconds"],
                                 "selected_fit_seconds": None,
                                 "selected_fit_time_unavailable": True,
    ...
    fields = ["dataset", "arm", "seed", "procedure", "split", "score", "improvement_over_f0", "coverage80",
              ...
              "trainable", "checkpoint_bytes", "optimizer_seconds_per_step", "fit_seconds", "hpo_fit_seconds",
              "selected_fit_seconds", "selected_fit_time_unavailable", "peak_cuda_allocated_bytes",
    ...
    summary = {"completed": True, "study": STUDY, "contract_sha256": contract_hash,
               ...
               "audit": {"all_neural_grid_fits": len(fit_costs), "selected_neural_forecasts": len(forecast_costs),
                         ...
                         "independent_fit_audit_path": independent_fit_audit["path"],
                         "independent_fit_audit_sha256": independent_fit_audit["sha256"],
                         "fixed_chronology": fixed_chronology,
