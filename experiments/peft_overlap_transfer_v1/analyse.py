...
def main():
    ...
    for entry in selections:
        ...
        starts.append(datetime.fromisoformat(json.loads((parent / 'guard/status.json').read_text())['started_at']).timestamp())
        rows.append({'dataset': job['dataset'], 'condition': job['condition'], 'arm': job['arm'], 'seed': job['seed'], 'lr': job['head_lr'], 'score': actual, 'f0_score': f0[job['dataset']], 'gain_pct_F0': 100*(f0[job['dataset']]-actual)/f0[job['dataset']], 'best_step': best, 'fit_seconds': fit['seconds'], 'trainable': fit['trainable']})
