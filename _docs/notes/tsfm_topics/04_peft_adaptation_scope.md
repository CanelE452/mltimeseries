...
A는 우선 설계할 후보다. 적응 범위 선택이 데이터에 따라 불안정하면 [B](05_peft_selection_reliability.md)를 검토한다. 적응 후 실제로 유지해야 하는 예측 능력의 손실이 보이면 [C](06_peft_forecasting_ability_preservation.md)를 검토한다. 세 가지를 동시에 구현하는 계획은 아니다.
